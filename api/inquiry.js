const _rlInq = new Map();
function rlInq(k,max,ms){ const n=Date.now(),r=_rlInq.get(k)||{c:0,t:n+ms}; if(n>r.t){r.c=0;r.t=n+ms;} r.c++; _rlInq.set(k,r); return r.c>max; }

export default async function handler(req, res) {
  res.setHeader('X-Content-Type-Options', 'nosniff');
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  if (req.method === 'OPTIONS') return res.status(200).end();
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const ip = req.headers['x-forwarded-for']?.split(',')[0] || 'unknown';
  if (rlInq(`inq:${ip}`, 5, 3600000)) return res.status(429).json({ error: 'Too many requests. Please wait.' });

  const { service, name, email, phone, website, budget, message, details, type } = req.body;
  if (!name || !email) return res.status(400).json({ error: 'Missing required fields' });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) return res.status(400).json({ error: 'Invalid email' });

  const isContact = type === 'contact';
  const LOGO = '';
  const serviceColor = isContact ? '#6366f1' : '#4f7cff';
  const accentBg = isContact ? '#f5f3ff' : '#f0f4ff';
  const accentBorder = isContact ? '#ddd6fe' : '#c7d7ff';
  const dateStr = new Date().toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'});
  const subject = isContact ? '💬 New Contact Message from ' + name : '🔔 New ' + (service||'Service') + ' Inquiry from ' + name;

  const detailRows = details ? Object.entries(details).map(([k,v]) =>
    '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5;width:35%">' + k + '</td>'
    + '<td style="padding:9px 14px;font-size:13px;color:#1a202c;border:1px solid #e8edf5">' + v + '</td></tr>'
  ).join('') : '';

  const html = [
    '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="margin:0;padding:0;background:#f0f2f5">',
    '<table width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f0f2f5;padding:20px 0">',
    '<tr><td align="center" style="padding:0 16px">',
    '<table cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:560px;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">',
    // HEADER
    '<tr><td style="background:linear-gradient(135deg,#0f1628 0%,#1a2d5a 100%);padding:20px 24px;border-bottom:3px solid ' + serviceColor + '">',
    '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>',
    '<td><table cellpadding="0" cellspacing="0" border="0"><tr>',
    '<td style="padding-right:10px"><img src="data:image/png;base64,' + LOGO + '" width="38" height="38" style="display:block;border-radius:9px;border:0"/></td>',
    '<td><div style="font-family:Arial,sans-serif;font-size:20px;font-weight:900;color:#fff">Uply<span style="color:#4f7cff">ncio</span></div>',
    '<div style="font-family:Arial,sans-serif;font-size:11px;color:#8892a4">Guest Posting Marketplace</div></td>',
    '</tr></table></td>',
    '<td align="right">',
    '<div style="font-family:Arial,sans-serif;font-size:10px;font-weight:700;color:#8892a4;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:5px">' + (isContact ? 'Contact Message' : 'New Inquiry') + '</div>',
    '<div style="font-family:Arial,sans-serif;font-size:11px;color:rgba(255,255,255,.4)">' + dateStr + '</div>',
    '</td></tr></table></td></tr>',
    // SERVICE BADGE
    service && !isContact ? '<tr><td style="padding:20px 24px 0"><div style="background:' + accentBg + ';border:1px solid ' + accentBorder + ';border-radius:10px;padding:14px 18px"><div style="font-size:10px;font-weight:700;color:' + serviceColor + ';text-transform:uppercase;letter-spacing:1px;font-family:Arial,sans-serif;margin-bottom:4px">Service Requested</div><div style="font-size:18px;font-weight:800;color:#1a202c;font-family:Arial,sans-serif">' + service + '</div></div></td></tr>' : '',
    // BODY
    '<tr><td style="padding:20px 24px">',
    '<p style="font-family:Arial,sans-serif;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin:0 0 10px">Client Details</p>',
    '<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:16px">',
    '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5;width:35%">Name</td><td style="padding:9px 14px;font-size:13px;font-weight:700;color:#1a202c;border:1px solid #e8edf5">' + name + '</td></tr>',
    '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5">Email</td><td style="padding:9px 14px;font-size:13px;border:1px solid #e8edf5"><a href="mailto:' + email + '" style="color:#4f7cff;font-weight:600;text-decoration:none">' + email + '</a></td></tr>',
    phone ? '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5">Phone</td><td style="padding:9px 14px;font-size:13px;color:#1a202c;border:1px solid #e8edf5">' + phone + '</td></tr>' : '',
    website ? '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5">Website</td><td style="padding:9px 14px;font-size:13px;border:1px solid #e8edf5"><a href="' + website + '" style="color:#4f7cff;text-decoration:none">' + website + '</a></td></tr>' : '',
    budget ? '<tr><td style="padding:9px 14px;font-size:13px;font-weight:600;color:#64748b;background:#f8faff;border:1px solid #e8edf5">Budget</td><td style="padding:9px 14px;font-size:13px;font-weight:700;color:#00c27a;border:1px solid #e8edf5">' + budget + '</td></tr>' : '',
    detailRows,
    '</table>',
    message ? '<p style="font-family:Arial,sans-serif;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin:0 0 10px">Message</p><div style="background:#f8faff;border:1px solid #e8edf5;border-left:3px solid ' + serviceColor + ';border-radius:0 8px 8px 0;padding:14px 16px;font-family:Arial,sans-serif;font-size:14px;color:#1a202c;line-height:1.8;margin-bottom:16px">' + message + '</div>' : '',
    '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px 18px;font-family:Arial,sans-serif;font-size:13px;color:#15803d">&#10003; Reply directly to this email to respond to <strong>' + name + '</strong></div>',
    '</td></tr>',
    // FOOTER
    '<tr><td style="background:linear-gradient(135deg,#0f1628 0%,#1a2d5a 100%);padding:16px 24px;border-radius:0 0 12px 12px">',
    '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>',
    '<td><p style="font-family:Arial,sans-serif;font-size:14px;font-weight:900;color:#fff;margin:0 0 3px">Uply<span style="color:#4f7cff">ncio</span></p><p style="font-family:Arial,sans-serif;font-size:10px;color:#8892a4;margin:0">Guest posting &amp; link building marketplace.</p></td>',
    '<td align="right"><p style="font-family:Arial,sans-serif;font-size:10px;color:rgba(255,255,255,.3);margin:0">info@uplyncio.com &nbsp;|&nbsp; uplyncio.com</p></td>',
    '</tr></table></td></tr>',
    '</table></td></tr></table></body></html>'
  ].filter(Boolean).join('');

  try {
    const r = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + process.env.RESEND_API_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: 'Uplyncio Inquiries <info@uplyncio.com>', to: ['info@uplyncio.com'], reply_to: email, subject, html })
    });
    const data = await r.json();
    if (!r.ok) return res.status(500).json({ error: data });
    return res.status(200).json({ success: true });
  } catch(e) {
    return res.status(500).json({ error: e.message });
  }
}